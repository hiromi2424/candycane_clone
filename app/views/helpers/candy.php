...
class CandyHelper extends AppHelper {
	...
	public function link($user, $options = array()) {
		if (isset($user['User'])) {
			$user = $user['User'];
		}

		return $this->Html->link($user['name'], array(
			'controller' => 'account',
			'action' => 'show',
			$user['id'],
		), $options);
...
#
	...
	public function toggle_link($name, $id, $options=array()) {
...
#	end
#
	public function syntax_highlight($name, $content) {
		// Use StyleHelper
...
#	end
#
	public function pagination_links_full($options = array()) {
...
#	end
#
	public function per_page_links($selected = null) {
...
#	def textilizable(*args)
	public function textilizable($text, $options=array()) {
...
#	end
	...
	protected function _replaceWikiLinks($already_matched) {
		/* Wiki links
		 * Examples:
		 * [[mypage]]
		 *	 [[mypage|mytext]]
...
#
	 ...
	 *
	 */
	public function lang_options_for_select($blank = true) {
		$list = array();
...
#	end
#
	public function back_url_hidden_field_tag($form) {
...
#	end
#
	public function progress_bar_auto($pcts, $options=array()) {
			$total = $pcts[0] + $pcts[1];
	...
	public function progress_bar($pcts, $options=array()) {
	...
	public function context_menu_link($name, $url, $options=array()) {
	...
	public function calendar_for($field_id) {
	...
	public function include_calendar_headers_tags() {
...
#
	 ...
	 * (eg. 'joe <joe@foo.bar>')
	 */
	public function avatar($user, $options = array()) {
...
#	end
	...
	public function cycle($one = "odd", $two = "even") {
		if($this->row++ % 2) {
			return $two;
		} else {
			return $one;
	...
	public function label_text($text) {
	...
	public function check_all_links($form_name) {
	 ...
	 */
	public function distance_of_time_in_words($begin, $end, $include_seconds=false) {
	...
	public function options_from_collection_for_select($list, $model, $value, $text, $none=false) {
		$options = array();
		if($none) {
			$options['none'] = $none;
		}
